...
VideoFragment.OnVideoSelectedListener{
	...
	private TextView headerBarTitle,txtPreview;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent() !=null)
		{
			...
			if (type ==1) {
				headerBarTitle.setTextColor(0xff018442);
				tv_cancel.setTextColor(0xff018442);
				headerBarBack.setImageResource(R.drawable.mc_btn_back_zhigu);
			}
		}
		...
		findViewById(R.id.tv_cancel).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				setResult(MediaChooser.RESULT_CLOSE_ALL, null);
				finish();
			}
		});
	}
	...
	public void onImageSelected(int count) {
		System.out.println("图片选择--" + mTabHost.getTabWidget().getChildAt(1) + ", " + count);
		if( mTabHost.getTabWidget().getChildAt(1) != null){
			if(count != 0){
				((TextView) mTabHost.getTabWidget().getChildAt(1).findViewById(android.R.id.title)).setText(getResources().getString(R.string.images_tab) + "111" + count);
			...
			if(count != 0){
				((TextView) mTabHost.getTabWidget().getChildAt(0).findViewById(android.R.id.title)).setText(getResources().getString(R.string.images_tab) + "222"  + count);
	...
	public void onBackPressed() {
		setResult(MediaChooser.RESULT_OK, null);
		super.onBackPressed();
	}
}
