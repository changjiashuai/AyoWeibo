...
import com.learnncode.mediachooser.async.Bimp;
...
public class PhotoPreviewActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mSelectedItems= getIntent().getStringArrayListExtra("key_img_list");
		

		Button photo_bt_exit = (Button) findViewById(R.id.photo_bt_exit);
		photo_bt_exit.setVisibility(View.GONE);
		
		Button photo_bt_del = (Button) findViewById(R.id.photo_bt_del);
		photo_bt_del.setVisibility(View.GONE);
		
		Button photo_bt_enter = (Button) findViewById(R.id.photo_bt_enter);
		photo_bt_enter.setBackgroundResource(R.drawable.bt_quxiao);
		...
		for (int i = 0; i < mSelectedItems.size(); i++) {
			...
			try {
				...
				initListViews(Bimp.revitionImageSize(mSelectedItems.get(i)));
			} catch (IOException e) {
