...
public class HttpScheduler {
	...
	public static <T> void start(final AyoRequest req, final BaseResponseDispatcher responseHandler, final BaseHttpCallback<T> callback){
		...
		if(req.stringEntity != null && !req.stringEntity.equals("")){
			//有entity形式的参数，截止到2016/4/11，还只有Okhttp支持
			if(!(req.worker instanceof HttpWorkerUseOkhttp))
				throw new RuntimeException("只有Okhttp支持string entity形式的提交");
		}

		if(req.files.size() > 0){
			//有文件上传，截止到2016/4/11，还只有Okhttp支持
			if(!(req.worker instanceof HttpWorkerUseOkhttp))
				throw new RuntimeException("只有Okhttp支持文件上传");
		}
